<template>
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div
      v-if="errorMessage"
      class="mb-6 bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-md"
    >
        ...
        <div class="flex items-center">
          <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
            <path
              ...
              1.414L10 11.414l1.293 1.293a1 1 
      ...
      <div class="flex justify-between items-start mb-6">
        <div>
          ...
          <div class="flex items-center gap-4 mb-4">
            <svg
              class="w-7 h-7 text-purple-300 shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
            ...
            <div class="flex flex-col justify-center">
              ...
              <span class="text-purple-100 text-base font-bold leading-tight truncate">
                {{ walletName }}
              </span>
            </div>
          </div>
        ...
        <div>
          <button
            class="mr-3 bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg font-medium transition-colors backdrop-blur-sm"
...
export default {
  ...
  methods: {
    ...
    async getWalletDetails() {
      const data = await getWallet(this.walletId);
      const wallet = data.GetWalletDetails;
      this.walletId = wallet.WalletId;
      this.walletName = wallet.Name;
      this.members = wallet.Members.map((user) => ({
        initials: getInitials(user.Fullname),
        name: user.Fullname,
        email: user.Email,
        amount: this.formatCurrency(user.AmountContributed ?? 0),
        role: user.Role || "Member",
        bgClass: user.Role === "Owner" ? "bg-yellow-600" : "bg-navy-600",
        roleBgClass: user.Role === "Owner" ? "bg-yellow-100" : "bg-purple-100",
        roleTextClass: user.Role === "Owner" ? "text-yellow-700" : "text-purple-700",
    ...
    async topUpWalletBalance() {
      try {
        ...
        const result = await topUpWallet(customerId, accountId, walletId, amount, `Top-up to wallet ${walletId}`);
        if (result.success) {
          alert("✅ Wallet top-up successful!");
          this.sharedBalance = `$${(
            parseFloat(this.sharedBalance.replace(/[$,]/g, "")) + amount
          ).toFixed(2)}`;
        ...
        alert("An unexpected error occurred during top-up.");
      } finally {
